...
This is the complete contract reference for the high-level
@racketmodname[xgboost] API. For a task-oriented walkthrough, start with the
@secref["getting-started"] chapter of the @secref["user-guide"] above.

The @racketmodfont{xgboost/foreign} module additionally exposes lower-level
DMatrix constructors for URI loading, dense array-interface input, CSR, CSC,
and columnar array-interface input, intended for callers that already work with
native-style buffers or XGBoost JSON array-interface strings. It also exposes
local DMatrix metadata and dataset operations such as feature names/types, uint
info, row slicing, binary DMatrix saving, and quantile-cut inspection. Booster
inspection APIs cover lifecycle queries, JSON config round-trips, attributes,
feature info, model dumps, and feature scores. CPU inplace prediction APIs
support dense, CSR, and columnar array-interface inputs for serving-style
prediction without constructing a DMatrix first. Custom objective training is
available by supplying gradient and Hessian vectors for one boosting iteration
at a time.
